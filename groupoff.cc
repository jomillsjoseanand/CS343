...
Groupoff::Groupoff( Printer &prt, unsigned int numStudents, unsigned int sodaCost, unsigned int groupoffDelay ) : 
    prt(prt), numStudents(numStudents), sodaCost(sodaCost), groupoffDelay(groupoffDelay), giftCards.resize(numStudents) {
}
...
void Groupoff::main() {
    ...
    while (assigned < numStudents) {
        ...
        if (!giftCards[student].available()) {
            giftCards[student].delivery(new WATCard(sodaCost));
